...
THESIS_TOPIC_RULES = [
    (
        ...
        "thesis_form",
        "form_requirement",
        ["kartu bimbingan skripsi", "bimbingan skripsi"],
    ),
    (
        ("pembimbingan", "bimbingan", "pembimbing"),
        "thesis_supervision",
        "thesis_sop",
        "procedure",
        ["pembimbingan skripsi", "dosen pembimbing", "SOP pembimbingan"],
    ),
    (
        ("persetujuan tema", "tema skripsi", "lembar persetujuan"),
        "thesis_topic_approval",
        "thesis_form",
        "form_requirement",
        ["lembar persetujuan tema", "persetujuan tema", "tema skripsi"],
    ),
    (
        ("pedoman penulisan", "format skripsi", "penulisan skripsi", "fpsi"),
        "thesis_writing_guideline",
        "thesis_guide",
        "rule_policy",
        ["pedoman penulisan skripsi", "skripsi FPSI", "format penulisan skripsi"],
    ),
    (
        ("yudisium",),
        "yudisium_and_graduation",
        "graduation_regulation",
        "rule_policy",
        ["yudisium", "wisuda", "aturan yudisium"],
    ),
    (
        ("wisuda", "wisudawan", "ijazah"),
        "graduation_ceremony",
        "graduation_announcement",
        "rule_policy",
        ["wisuda 2025", "syarat wisuda", "peserta wisuda", "pengambilan ijazah"],
    ),
]
...
def fallback_analyze_query(user_query: str) -> dict:
    ...
    document_year = "2025" if "2025" in query else "2022" if "2022" in query else "2018" if "2018" in query else ""

    for triggers, topic, document_type, query_intent, keywords in THESIS_TOPIC_RULES:
    ...
    return {
        ...
        "rerank_keywords": ["skripsi", "wisuda", "yudisium"],
        "reason": "General thesis/graduation fallback.",
    }
...
ANALYZER_CONFIG = DomainAnalyzerConfig(
    ...
    default_metadata_filter_fields=("topic", "document_year"),
    ...
    for doc, distance in results:
        ...
        if "wisuda" in query_lower:
            if topic in {"graduation_ceremony", "yudisium_and_graduation", "graduation_requirement"}:
                bonus += 5
            if "calon wisudawan" in text or "pendaftaran wisuda" in text:
                bonus += 6
            if "peserta wisuda" in text:
                bonus += 4
            if "pengambilan ijazah" in text:
                bonus += 4
